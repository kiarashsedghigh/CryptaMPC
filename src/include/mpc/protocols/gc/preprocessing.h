...
#include "utils/prp.h"
#include "feq.h"
#include "mpc/protocols/gc/leaky_deltaot.h"
#include "helper.h"
...
class Fpre {
	public:
		...
		Fpre(T * in_io, int in_party, int bsize = 1000) {
			...
			for(int i = 0; i < THDS; ++i) {
				usleep(1000);
				io[i] = new T(in_io->m_ip.c_str(), in_io->m_port, in_io->m_is_server);

				usleep(1000);
				io2[i] = new T(in_io->m_ip.c_str(), in_io->m_port, in_io->m_is_server);
...
//			cout << size<<"\t"<<batch_size<<"\n";
		...
		void check(block * MAC, block * KEY, int length, int I) {
			...
			if(party == ALICE) {
				helper_send_data(local_io, G, sizeof(block)*length);
				helper_recv_data(local_io, GR, sizeof(block)*length);
			} else {
				helper_recv_data(local_io, GR, sizeof(block)*length);
				helper_send_data(local_io, G, sizeof(block)*length);
			}
			...
			if(party == ALICE) {
				helper_send_bool(local_io, d, length);
				helper_recv_bool(local_io, dR,length);
			} else {
				helper_recv_bool(local_io, dR, length);
				helper_send_bool(local_io, d, length);
			}
		...
		void combine(block S, int I, block * MAC, block * KEY, int length, int bucket_size, block * MAC_res, block * KEY_res) {
			...
			if(party == ALICE) {
				helper_send_bool(io[I], data, length*bucket_size);
				helper_recv_bool(io[I], data2, length*bucket_size);
			} else {
				helper_recv_bool(io[I], data2, length*bucket_size);
				helper_send_bool(io[I], data, length*bucket_size);
			}
